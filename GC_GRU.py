...
class GC_GRU(nn.Module):
    def __init__(self, hist_len, pred_len, in_dim, city_num, batch_size, device, edge_index):
        ...
        self.in_dim = in_dim
        self.hid_dim = 64 #기존: 32, 64가 훨씬 성능 좋음 (역할 확인)
        self.out_dim = 1
        self.gcn_out = 20 #기존:1 , 13가 성능 좋음 (역할 확인)
        self.conv = ChebConv(self.in_dim, self.gcn_out, K=2) #K= 2(성능 좋음), 3, 4
    ...
    def forward(self, pm25_hist, feature):
       ...
       # cn = c0
        ...
        for i in range(self.pred_len):
            ...
            x_gcn = F.sigmoid(self.conv(x_gcn, self.edge_index))# 수정
           # x_gcn = F.dropout(x_gcn, self.dropout,training=self.training) #dropout 추가함
            x_gcn = x_gcn.view(self.batch_size, self.city_num, -1)
            x = torch.cat((x, x_gcn), dim=-1)
            
            hn = self.gru_cell(x, hn)
           # hn, cn = self.gru_cell(x, (hn, cn))
